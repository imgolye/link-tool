...
import org.springlink.core.tool.utils.BeanUtil;
import org.springlink.core.tool.utils.Func;
import org.springlink.core.tool.utils.PlaceholderUtil;
...
public class LinkDataScopeHandler implements DataScopeHandler {

	private final ScopeModelHandler scopeModelHandler;

	@Override
	public String sqlCondition(String mapperId, DataScopeModel dataScope, LinkUser linkUser, String originalSql) {

		//数据权限资源编号
		String code = dataScope.getResourceCode();

		//根据mapperId从数据库中获取对应模型
		DataScopeModel dataScopeDb = scopeModelHandler.getDataScopeByMapper(mapperId, linkUser.getRoleId());
		...
		if (DataScopeEnum.ALL == scopeTypeEnum || StringUtil.containsAny(linkUser.getRoleName(), RoleConstant.ADMIN)) {
			return null;
		} else if (DataScopeEnum.CUSTOM == scopeTypeEnum) {
			whereSql = PlaceholderUtil.getDefaultResolver().resolveByMap(dataScope.getScopeValue(), BeanUtil.toMap(linkUser));
		} else if (DataScopeEnum.OWN == scopeTypeEnum) {
			ids.add(linkUser.getUserId());
		} else if (DataScopeEnum.OWN_DEPT == scopeTypeEnum) {
			ids.addAll(Func.toLongList(linkUser.getDeptId()));
		} else if (DataScopeEnum.OWN_DEPT_CHILD == scopeTypeEnum) {
			List<Long> deptIds = Func.toLongList(linkUser.getDeptId());
			ids.addAll(deptIds);
