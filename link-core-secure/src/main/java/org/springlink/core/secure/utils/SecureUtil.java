package org.springlink.core.secure.utils;
...
import org.springlink.core.launch.constant.TokenConstant;
import org.springlink.core.secure.LinkUser;
import org.springlink.core.secure.TokenInfo;
import org.springlink.core.secure.constant.SecureConstant;
import org.springlink.core.secure.exception.SecureException;
import org.springlink.core.secure.provider.IClientDetails;
import org.springlink.core.secure.provider.IClientDetailsService;
...
public class SecureUtil {
	private static final String LINK_USER_REQUEST_ATTR = "_LINK_USER_REQUEST_ATTR_";
	...
	public static LinkUser getUser() {
		...
		// 优先从 request 中获取
		Object linkUser = request.getAttribute(LINK_USER_REQUEST_ATTR);
		if (linkUser == null) {
			linkUser = getUser(request);
			if (linkUser != null) {
				// 设置到 request 中
				request.setAttribute(LINK_USER_REQUEST_ATTR, linkUser);
			}
		}
		return (LinkUser) linkUser;
	}
	 ...
	 * @return LinkUser
	 */
	public static LinkUser getUser(HttpServletRequest request) {
		...
		linkUser.setClientId(clientId);
		linkUser.setUserId(userId);
		linkUser.setTenantId(tenantId);
		linkUser.setAccount(account);
		linkUser.setRoleId(roleId);
		linkUser.setDeptId(deptId);
