...
import org.springlink.core.cloud.annotation.ApiVersion;
import org.springlink.core.cloud.annotation.UrlVersion;
...
/**
 * 支持 link-boot 的 版本 处理
 *
 * @author gaolei
 * @see org.springlink.core.cloud.annotation.UrlVersion
 * @see org.springlink.core.cloud.annotation.ApiVersion
 */
public class LinkSpringMvcContract extends SpringMvcContract {

	public LinkSpringMvcContract(List<AnnotatedParameterProcessor> annotatedParameterProcessors, ConversionService conversionService) {
	...
	protected void processAnnotationOnMethod(MethodMetadata data, Annotation methodAnnotation, Method method) {
		if (RequestMapping.class.isInstance(methodAnnotation) || methodAnnotation.annotationType().isAnnotationPresent(RequestMapping.class)) {
			...
			if (apiVersion != null && StringUtil.isNotBlank(apiVersion.value())) {
				LinkMediaType LinkMediaType = new LinkMediaType(apiVersion.value());
				data.template().header(HttpHeaders.ACCEPT, LinkMediaType.toString());
			}
		}
	}
